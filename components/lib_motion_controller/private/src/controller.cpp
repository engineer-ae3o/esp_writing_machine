...
namespace gcode::controller {
    ...
    void controller_t::init(const types::config_t& config) {
    ...
    void controller_t::create_session(const types::session_t& session) {
        ...
        types::event_t event{types::event_t::SESSION_CREATED};
    ...
    void controller_t::send_event(types::event_t event) {
        ASSERT(m_is_initialized);
        // The only way to create a session is through the `start_session()` function
        if (event == types::event_t::SESSION_CREATED) return;
        xQueueSend(m_event_queue, &event, portMAX_DELAY);
    }

    types::state_t controller_t::get_state() {
        ASSERT(m_is_initialized);
        return m_state;
    }

    constexpr const char* controller_t::event_to_string(types::event_t event) {
        switch (event) {
            case types::event_t::NO_EVENT:        return "NO_EVENT";
            case types::event_t::SESSION_CREATED: return "SESSION_CREATED";
            case types::event_t::PAUSE_SESSION:   return "PAUSE_SESSION";
            case types::event_t::START_SESSION:   return "START_SESSION";
            case types::event_t::STOP_SESSION:    return "STOP_SESSION";
            default:                              return "UNKNOWN";
        }
    }
    
    constexpr const char* controller_t::controller_state_to_string(types::state_t state) {
        switch (state) {
            case types::state_t::SLEEPING: return "SLEEPING";
            case types::state_t::RUNNING:  return "RUNNING";
            case types::state_t::PAUSED:   return "PAUSED";
            case types::state_t::STOPPED:  return "STOPPED";
            default:                       return "UNKNOWN";
        }
    }

    // Private functions
    controller_t::controller_t(const types::config_t& config) {
        init(config);
    }
    ...
    void controller_t::cleanup() {
        ...
        types::event_t event = types::event_t::STOP_SESSION;
        xQueueSend(m_event_queue, &event, portMAX_DELAY);
        m_is_initialized = false;
    }
    
    types::error_t controller_t::get_error_from_parser_error(parser::error_t error) {
        switch (error) {
            case parser::error_t::INVALID_COMMAND:   return types::error_t::INVALID_COMMAND;
            case parser::error_t::INVALID_SYNTAX:    return types::error_t::INVALID_SYNTAX;
            case parser::error_t::MISSING_PARAMETER: return types::error_t::MISSING_PARAMETER;
            default:                                 return types::error_t::UNKNOWN;
        }
    }
    ...
    void controller_t::planner_task(void* arg) {
        ...
        types::event_t event{types::event_t::NO_EVENT};
        types::session_done_t session_done{};
        ...
        while (!driver->m_shutdown_requested) {
            switch (driver->m_state) {
                case types::state_t::SLEEPING:
                    ...
                    if (event != types::event_t::SESSION_CREATED) {
                    ...
                    file_handle = fopen(driver->m_session.file_path.data(), "r");
                    if (!file_handle) {
                        session_done.error = types::error_t::FILE_NOT_FOUND;
                ...
                case types::state_t::RUNNING: {
                    ...
                    if (!fgets(gcode_line.data(), gcode_line.size(), file_handle)) {
                        ...
                        // A file IO error occurred
                            ...
                            session_done.error = types::error_t::FILE_READ_ERROR;
                            utils::log<config::log_level_t::ERROR>("File IO error");
                        }
                        // Most likely won't get here, but incase
                        else {
                            session_done.error = types::error_t::UNKNOWN;
                ...
                case types::state_t::PAUSED:
                    ...
                    xQueueReceive(driver->m_event_queue, &event, portMAX_DELAY);
                    if ((event == types::event_t::START_SESSION) || (event == types::event_t::RESUME_SESSION)) {
                        driver->m_state = types::state_t::RUNNING;
