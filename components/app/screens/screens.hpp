...
namespace display {
    ...
    void create_scale_screen(float current_scale = 0);
    void create_speed_screen(float current_speed = 0);
    ...
    // Toasts
    // Gets created on the active screen. A new toast replaces any existing
    // one. No auto dismiss. Caller clears manually
    ...
    // Modals
    // Gets created on the active screen with a 70% dim overlay behind them.
    // `on_ok` is called after the modal is dismissed. May be nullptr.
    // Clears any previously active modal
